# -*- coding: utf-8 -*-
"""把 build_xingzheng.py 产出的 feishu_records.json（434 条李佳行政法）
按飞书字段类型格式化后 batch_create 写入多维表格（当前行政法已为 0 条，全新导入）。
读 D:\\测试\\.env 凭据。
流程:
...
SRC = os.path.join(ROOT, "quiz_output", "2026-08-10_行政法李佳重导", "feishu_records.json")
